...
export class ScaleHandle extends StandardHandle {
  ...
  async generateBaseGeometry() {
    ...
    this._group.getManager()._cubeMesh = await utility.createCubeMesh(
      viewer,
      new Communicator.Point3(0, 0, 0.15),
      0.025
    );
    ...
    this._group.getManager()._cubeMesh2 = await this._group
      .getViewer()
  ...
  async show() {
    ...
    let myMeshInstanceData = new Communicator.MeshInstanceData(
      this._group.getManager()._cubeMesh
    );
    await viewer.model.createMeshInstance(myMeshInstanceData, this._nodeid);

    myMeshInstanceData = new Communicator.MeshInstanceData(
      this._group.getManager()._cubeMesh2
    );
    await viewer.model.createMeshInstance(myMeshInstanceData, this._nodeid);
  ...
  async handleMouseMove(event) {
    ...
    for (let i = 0; i < this._startTargetMatrices.length; i++) {
      ...
      if (this._group._relative) {
        newnormal3 = utility.rotateNormal(
          viewer.model.getNodeMatrix(this._nodeid),
          new Communicator.Point3(0, 0, 1)
        );
        let smat = new Communicator.Matrix();
        smat.setScaleComponent(
          1 + newnormal3.x * d,
          1 + newnormal3.y * d,
          1 + newnormal3.z * d
        );
        ...
        resmatrix3 = Communicator.Matrix.multiply(
          resmatrix2,
          this._startTargetMatrices[i]
        );
      } else {
        newnormal3 = utility.rotateNormal(
          viewer.model.getNodeNetMatrix(this._nodeid),
          new Communicator.Point3(0, 0, 1)
        );
        let center = Communicator.Matrix.inverse(
          viewer.model.getNodeNetMatrix(this._group._targetNodes[i])
