...
export class TranslateHandle extends StandardHandle {
  ...
  async generateBaseGeometry() {
    ...
    let meshData = Communicator.Util.generateConeCylinderMeshData(
      0.0045,
      10,
      0.15,
      0.0195,
    ...
    this._group.getManager()._arrowMesh = await this._group
      .getViewer()
      .model.createMesh(meshData);
    let meshData2 = Communicator.Util.generateConeCylinderMeshData(
      0.0,
      10,
      0.15,
      0.0195,
    ...
    this._group.getManager()._arrowMesh2 = await this._group
      .getViewer()
      .model.createMesh(meshData2);
  }
  ...
  async show() {
    ...
    let myMeshInstanceData = new Communicator.MeshInstanceData(
      this._group.getManager()._arrowMesh
    );
    ...
    let myMeshInstanceData2 = new Communicator.MeshInstanceData(
      this._group.getManager()._arrowMesh2
    );
    let nodeid2 = await viewer.model.createMeshInstance(
      myMeshInstanceData2,
      this._nodeid
    );
  ...
  async handleMouseMove(event) {
    ...
    let spos = utility.nearestPointOnLine(
      newpos2,
      newnormal2,
      this._startPosition
    );

    let pointonline = utility.getClosestPoint(
      viewer,
      newpos2,
      newnormal2,
      event.getPosition()
